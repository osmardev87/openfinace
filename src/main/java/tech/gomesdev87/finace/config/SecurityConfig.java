...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf(csrf -> csrf.disable()).cors(Customizer.withDefaults())
                ...
                .authorizeHttpRequests(auth -> auth.requestMatchers(HttpMethod.POST, "/auth/login")
                        ...
                        .requestMatchers("/doc", "/doc/**", "/swagger-ui/**", "/v3/api-docs/**",
                        ...
                        // Frontend!
                        .requestMatchers("/", "/index.html", "/login.html", "/index2.html",
                                "/etiqueta.html", "/catalago.html", "/*.css", "/*.js", "/manifest.json", "/*.png", "/*.ico", "/favicon.ico", "/assets/**")
    ...
    public JwtEncoder jwtEncoder() {
        validarTamanhoChave();
        SecretKey secretKey = obterSecretKey();
        JWK jwk = new OctetSequenceKey.Builder(secretKey).keyID("conectafinace-secret-key")
                .algorithm(JWSAlgorithm.HS256).keyUse(com.nimbusds.jose.jwk.KeyUse.SIGNATURE)
                .build();
